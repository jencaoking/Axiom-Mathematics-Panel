...
class FractalGPUExplorer(QWidget):
    ...
    def _build_ui(self):
        ...
        html_path = os.path.abspath(os.path.join(
            os.path.dirname(__file__), '..', 'plugins', 'plugin_3d_viewer', 'web', 'fractal_gpu.html'
        ))
    ...
    def wheelEvent(self, event):
        ...
        if self.zoom < 0.1: self.max_iter = 512
        if self.zoom < 0.001: self.max_iter = 1024
        if self.zoom < 0.00001: self.max_iter = 2048
