...
namespace MathLab.CSharpEngine
{
    public class FastGeometry
    {
        ...
        /// 返回平铺数组 [x, y]，无交点返回空数组
        /// </summary>
        public double[] SolveLineLine(double a1, double b1, double c1, double a2, double b2, double c2)
        {
        ...
        /// 返回平铺数组 [x1, y1, x2, y2, ...]
        /// </summary>
        public double[] SolveLineCircle(double a, double b, double c, double cx, double cy, double r)
        {
            double n2 = a * a + b * b;
            if (n2 < Epsilon) return new double[0];

            double k = a * cx + b * cy + c;
            double n = Math.Sqrt(n2);
            double d = Math.Abs(k) / n;

            if (d > r + Epsilon) return new double[0];
        ...
        /// 返回平铺数组 [x1, y1, x2, y2, ...]
        /// </summary>
        public double[] SolveCircleCircle(double cx1, double cy1, double r1, double cx2, double cy2, double r2)
        {
            double dx = cx2 - cx1;
            double dy = cy2 - cy1;
            double d = Math.Sqrt(dx * dx + dy * dy);

            if (d > r1 + r2 || d < Math.Abs(r1 - r2)) return new double[0];
            if (d < Epsilon && Math.Abs(r1 - r2) < Epsilon) return new double[0];
